...
Thread::Thread(char* threadName, int threadID){
    ...
    priority = kernel -> priority[threadID];     
    accumulated_ticks = 0;                    
    burst_start_time = 0; 
    approx_burst_time = 0;
    last_accumulated_ticks = 0;             // mp3


    // DEBUG(dbgProc, "thread " << threadName << " init with priority " << priority)
    stackTop = NULL;
    stack = NULL;
    status = JUST_CREATED;
...
void Thread::setStatus(ThreadStatus st) { 
    ...
    if(stats){
        ...
        }else if(st == READY && status == RUNNING){
            accumulated_ticks += stats -> totalTicks - burst_start_time;
        }
        if(st == READY){
            wait_tick = stats -> totalTicks;
        }
    }
    status = st; 
}
...
//----------------------------------------------------------------------
//  Thread::update_approx_burst_time 
//  mp3
//  called at Semaphore::P()
//  updates approximate burst time
//----------------------------------------------------------------------
void Thread::update_approx_burst_time(){
    Statistics *stats = kernel -> stats;
    if(stats){
        accumulated_ticks += stats -> totalTicks - burst_start_time;
        float next_approx_burst_time = (approx_burst_time + accumulated_ticks)/2;
        // DEBUG(dbgProc, "burst_time : " << burst_time << ", now : " << stats -> totalTicks << ", burst_start time : " << burst_start_time);
        
        DEBUG(dbgProc, "[D] Tick [" << stats -> totalTicks << "]: Thread [" << ID 
                            << "] update approximate burst time, from: [" << approx_burst_time
                            << "], add [" << accumulated_ticks
                            << "], to [" << next_approx_burst_time << "]");
        approx_burst_time = next_approx_burst_time;

        // refresh queue
        if(kernel -> scheduler -> readyList_l1 -> IsInList(this)){
            kernel -> scheduler -> readyList_l1 -> Remove(this);
            kernel -> scheduler -> readyList_l1 -> Insert(this);
        }
    }
}
...
//  Thread::add_priority 
//  mp3
//  updates priority from aging policy
//----------------------------------------------------------------------
void Thread::add_priority(int x) { 
    int next_priority = min(priority + x, MaxPriority); 
    Statistics *stats = kernel -> stats;
    if(priority < MaxPriority)
        DEBUG(dbgProc, "[C] Tick [" << stats -> totalTicks << "]: Thread [" << ID << "] changes its priority from [" << priority << "] to [" << next_priority << "]");
    priority = next_priority;
    
    // refresh queue
    if(kernel -> scheduler -> readyList_l2 -> IsInList(this)){
        kernel -> scheduler -> readyList_l2 -> Remove(this);
        kernel -> scheduler -> readyList_l2 -> Insert(this);
    }
}
...
Thread::Sleep (bool finishing)
{
    ...
    if(finishing){      // mp3 add burst time for main when ending
        accumulated_ticks += kernel->stats->totalTicks - burst_start_time;
        // DEBUG(dbgProc, "thread sleep update bt += " << kernel->stats->totalTicks - burst_start_time)
    }
    status = BLOCKED;
