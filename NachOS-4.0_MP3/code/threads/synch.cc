...
Semaphore::P()
{
    ...
    while (value == 0) { 		// semaphore not available
        ...
        currentThread -> update_approx_burst_time(); // mp3
        currentThread -> last_accumulated_ticks = currentThread -> accumulated_ticks;
        currentThread -> accumulated_ticks = 0;
...
Semaphore::V()
{
    ...
    if (!queue->IsEmpty()) {  // make thread ready.
        Thread *nextThread = queue->RemoveFront();
        kernel->scheduler->ReadyToRun(nextThread); // mp3
    }
    value++;
