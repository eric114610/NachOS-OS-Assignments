...
ExceptionHandler(ExceptionType which)
{
	...
	switch(type) {
		...
		{
			unsigned int val2;
			kernel->currentThread->space->Translate(val, &val2, FALSE);
			char *msg = &(kernel->machine->mainMemory[val2]);
		...
		val = kernel->machine->ReadRegister(4);
		{
			unsigned int val2;
			kernel->currentThread->space->Translate(val, &val2, FALSE);
			char *filename = &(kernel->machine->mainMemory[val2]);
		//cout << filename << endl;
		...
		case SC_Write:
			...
			unsigned int val2;
			kernel->currentThread->space->Translate(val, &val2, FALSE);
			char *buffer = &(kernel->machine->mainMemory[val2]);
